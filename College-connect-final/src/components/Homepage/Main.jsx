...
import { getPost,addLike } from "../../routes/routes";
...
function Main(props) {
	...
	const likePost=async(postId)=>{
		// async function funti(){
			...
			const {data}=await addLike(payLoad);
			setActive(!isActive); 
		// }
		// funti();
	}
				...
				{res  &&
					res.map((article) => (
						<Article >
							<SharedActor>
								<a>
									{article.profile ? <img src={article.profile} alt="" /> : <img src="/images/user.svg" alt="" />}
									<div>
							...
							<SocialCount>
								{/* like and comment */}
								<button onClick={()=> likePost(article._id)} className={isActive ? "active": null} >
